...
//#endregion
//#region node_modules/@paperback/types/lib/impl/URL.js
/**
	...
	*/
	function parseURL(url) {
		...
		if (!match) throw new Error("Invalid URL string provided.");
		if (match[1] !== void 0 && match[1] !== "") components.protocol = match[1];
		if (match[2] !== void 0 && match[2] !== "") {
			...
			if (hostPort !== "") if (hostPort.startsWith("[")) {
				const closingBracketIndex = hostPort.indexOf("]");
				if (closingBracketIndex === -1) throw new Error("Invalid IPv6 address in URL update.");
				components.hostname = hostPort.substring(0, closingBracketIndex + 1);
				const portPart = hostPort.substring(closingBracketIndex + 1);
				if (portPart.startsWith(":")) components.port = portPart.substring(1);
			} else {
		...
		if (match[3] !== void 0 && match[3] !== "") components.path = match[3].startsWith("/") ? match[3] : `/${match[3]}`;
		if (match[4] !== void 0) {
			const query = {};
		...
		if (match[5] !== void 0) components.fragment = match[5];
		return components;
	}
	/**
	...
	var URL$1 = class {
		...
		*/
		constructor(url) {
			...
			_defineProperty(this, "queryItems", void 0);
			_defineProperty(this, "fragment", void 0);
			const components = parseURL(url);
			if (!components.hostname || !components.protocol) throw new Error("URL Hostname and Protocol are required");
		...
		*/
		toString() {
			...
			if (this.username !== void 0 && this.username !== "") {
				url += this.username;
				if (this.password !== void 0 && this.password !== "") url += `:${this.password}`;
				url += "@";
			}
			url += this.hostname;
			if (this.port !== void 0 && this.port !== "") url += `:${this.port}`;
			if (this.path !== "") url += this.path.startsWith("/") ? this.path : `/${this.path}`;
			if (this.queryItems !== void 0) {
				const queryKeys = Object.keys(this.queryItems);
				const params = [];
				if (queryKeys.length > 0) for (const key of queryKeys) {
					const value = this.queryItems[key];
					if (Array.isArray(value)) for (const v of value) params.push(`${encodeURIComponent(key)}=${encodeURIComponent(v)}`);
					else if (value !== void 0) params.push(`${encodeURIComponent(key)}=${encodeURIComponent(value)}`);
				}
				url += `?${params.join("&")}`;
			}
			if (this.fragment !== void 0) url += `#${this.fragment}`;
			return url;
		}
		/**
		...
		*/
		update(input) {
			let components;
			if (typeof input === "string") components = parseURL(input);
			else components = input;
			if (components.protocol !== void 0) this.setProtocol(components.protocol);
			if (components.username !== void 0) this.setUsername(components.username);
			if (components.password !== void 0) this.setPassword(components.password);
			if (components.hostname !== void 0) this.setHostname(components.hostname);
			if (components.port !== void 0) this.setPort(components.port);
			if (components.path !== void 0) this.setPath(components.path);
			if (components.queryItems !== void 0) this.setQueryItems(components.queryItems);
			if (components.fragment !== void 0) this.setFragment(components.fragment);
			return this;
		}
	};
...
//#endregion
//#region src/generic/forms/search.ts
	var MangaWorldAdvancedSearchForm = class extends AdvancedSearchForm {
		constructor(searchQuery) {
			super();
			_defineProperty(this, "searchMetadata", void 0);
			if (searchQuery.metadata !== void 0) this.searchMetadata = searchQuery.metadata;
			else {
				const def_type = Application.getState("def_type") ?? [];
				const hyde_type = Application.getState("hide_type") ?? [];
				const hide_tags = Application.getState("hide_tags") ?? [];
				this.searchMetadata = {
					type: Object.fromEntries([...def_type.map((k) => [k, "included"]), ...hyde_type.map((k) => [k, "excluded"])]),
		...
		getSections() {
			return [
				Section("type", [TriStateSelectRow("type", {
					title: "Tipologia",
					value: this.searchMetadata.type ?? {},
					layout: "list",
					allowExclusion: true,
					allowEmptySelection: true,
					items: filter.getMangaTypeFilter().map((x) => ({
						id: x.id,
						title: x.value
					})),
				...
				Section("genres", [TriStateSelectRow("genres", {
					...
					layout: "list",
					items: filter.getGenreFilter().map((x) => ({
						id: x.id,
						title: x.value
					})),
					allowExclusion: true,
					allowEmptySelection: true,
					onValueChange: Application.Selector(this, "handleGenreChange")
				})]),
				Section("year", [StepperRow(`year`, {
					title: "Anno",
				...
				Section("status", [SelectRow("status", {
					...
					minItemCount: 0,
					maxItemCount: 1,
					options: filter.getStatusFilter().map((x) => ({
						id: x.id,
						title: x.value
					})),
...
//#endregion
//#region src/generic/forms/settings.ts
	var Forms = class extends Form {
		constructor(manga_source) {
			super();
			_defineProperty(this, "manga_source", void 0);
	...
	var FilterSettings = class extends Form {
		constructor(..._args) {
			super(..._args);
			_defineProperty(this, "genres", filter.getGenreFilter().map(({ value, ...rest }) => ({
				title: value,
				...rest
			})));
			_defineProperty(this, "mangaTypes", filter.getMangaTypeFilter().map(({ value, ...rest }) => ({
				title: value,
				...rest
			})));
		}
		async updateValue(value, filter) {
		...
		getSections() {
			return [Section({
				...
				onValueChange: Application.Selector(this, "handleHideTagsStatusChange")
			}), SelectRow("hide_type", {
				...
				onValueChange: Application.Selector(this, "handleHideTypeStatusChange")
			})]), Section({
				id: "default_settings",
				...
				minItemCount: 0,
				maxItemCount: 1,
				onValueChange: Application.Selector(this, "handleDefTypeStatusChange")
			})])];
		}
		getHideTagsStatus() {
	...
	var FavSettings = class extends Form {
		constructor(..._args2) {
			super(..._args2);
			_defineProperty(this, "genres", filter.getGenreFilter().map(({ value, ...rest }) => ({
				title: value,
		...
		async updateToggleValue(value, filter) {
			Application.setState(value, filter);
			this.reloadForm();
		...
		getSections() {
			return [Section({
				...
				onValueChange: Application.Selector(this, "handleFavStatusChange")
				...
				minItemCount: 0,
				maxItemCount: 3,
				onValueChange: Application.Selector(this, "handleFavTagsNewStatusChange"),
				isHidden: !this.getFavStatus()
			})])];
		}
		getFavStatus() {
			return Application.getState("fav_section_enabled") ?? true;
		}
		async handleFavStatusChange(value) {
			await this.updateToggleValue(value, "fav_section_enabled");
		}
		getFavTagsNewStatus() {
			return Application.getState("fav_tags_new") ?? [];
		}
		async handleFavTagsNewStatusChange(value) {
			await this.updateValue(value, "fav_tags_new");
			Application.invalidateDiscoverSections();
		}
	};
	var HomeSettings = class extends Form {
		async updateToggleValue(value, filter) {
			Application.setState(value, filter);
			this.reloadForm();
			Application.invalidateDiscoverSections();
		...
		getPopularStatus() {
			return Application.getState("popular_section_enabled") ?? true;
		}
		async handlePopularStatusChange(value) {
			await this.updateToggleValue(value, "popular_section_enabled");
		}
		getMeseStatus() {
			return Application.getState("mese_section_enabled") ?? true;
		}
		async handleMeseStatusChange(value) {
			await this.updateToggleValue(value, "mese_section_enabled");
		}
		getMostReadStatus() {
			return Application.getState("most_read_section_enabled") ?? true;
		}
		async handleMostReadStatusChange(value) {
			await this.updateToggleValue(value, "most_read_section_enabled");
		}
		getUpdateStatus() {
			return Application.getState("update_section_enabled") ?? true;
		}
		async handleUpdateStatusChange(value) {
			await this.updateToggleValue(value, "update_section_enabled");
		}
		getNewStatus() {
			return Application.getState("new_section_enabled") ?? true;
		}
		async handleNewStatusChange(value) {
			await this.updateToggleValue(value, "new_section_enabled");
		}
		getTypeStatus() {
			return Application.getState("type_section_enabled") ?? true;
		}
		async handleTypeStatusChange(value) {
			await this.updateToggleValue(value, "type_section_enabled");
		}
		getGenreStatus() {
			return Application.getState("genre_section_enabled") ?? true;
		}
		async handleGenreStatusChange(value) {
			await this.updateToggleValue(value, "genre_section_enabled");
		}
	};

//#endregion
//#region src/generic/network.ts
	var Requests = class {
		constructSearchRequestURL(page, query = {
			title: "",
			metadata: {}
		}, sorting, source) {
			const genres = query.metadata?.genres ?? {};
			const types = query.metadata?.type ?? {};
			const generi = [];
			const generi_esclusi = [];
			const tipi_esclusi = [];
			const tipologia = [];
			for (const tag of Object.entries(genres)) {
				if (tag[1] == "included") generi.push(tag[0]);
				if (tag[1] == "excluded") generi_esclusi.push(filter.getGenreFilter().find((item) => item.id === tag[0])?.value ?? "");
			}
			if (types && typeof types === "object") for (const tag of Object.entries(types)) {
			...
			const url = new URL$1(source.base_url).addPathComponent("archive");
			if (query.title.toString().length > 0) url.setQueryItem("keyword", query.title.toString() ?? "");
			...
			return {
				url: url.toString(),
				excluded: {
					generi: generi_esclusi,
					tipi: tipi_esclusi
				}
			};
		}
		async parseFilters(source) {
			return Application.arrayBufferToUTF8String(await source.requestManager.fetchPage(`${source.base_url}/archive`));
		}
		async parseLastMangaAddedTagsSectionRequests(page, source, favTags) {
			let html = "";
			const tags = favTags ? Application.getState("fav_tags_new").join("&genre=") : "";
			if (page > 1) {
			...
			} else html = Application.arrayBufferToUTF8String(await source.requestManager.fetchPage(`${source.base_url}/archive?sort=newest&page=${page}&genre=${tags}`));
			return html;
		}
		async parsePopularSectionRequests(page, source) {
			let html = "";
			if (page > 1) {
				const data = (await Application.scheduleRequest({
				...
				html = Application.arrayBufferToUTF8String(data);
			} else html = Application.arrayBufferToUTF8String(await source.requestManager.fetchPage(`${source.base_url}/archive?sort=most_read&page=${page}`));
			return html;
		}
		async getSearchResultsRequests(url) {
			const data = (await Application.scheduleRequest({
				url,
				method: "GET"
			}))[1];
...
//#region src/generic/parsers.ts
	var Parsers = class {
		...
		*/
		parseMangaDetails(mangaInfo, mangaId, shareURL, source) {
			const entry = mangaInfo.find((e) => e.kind === "manga");
			if (!entry) throw new Error("Nessun dato 'manga' trovato nel JSON");
			const parsed = entry.data.manga;
			const tagsArray = jsonParser.mapGenresToTags(parsed.genres);
			const rating = source.defaultContentRating === ContentRating.ADULT ? ContentRating.ADULT : tags.getRating(parsed.genres?.map((g) => g.name) ?? []);
			const tagSections = [{
				id: "genres",
				title: "genres",
				tags: tagsArray
		...
		baseChapterData(chapter, sourceManga) {
			return {
		...
		*/
		parseChapters(items, sourceManga) {
			return items.filter((item) => item.kind === "chapter").flatMap((item) => {
				const elements = item.data.pages;
				const volumeChapters = (elements.volumes ?? []).flatMap((volume) => volume.chapters.map((chapter) => ({
				...
				const singleChapters = (elements.singleChapters ?? []).map((chapter) => this.baseChapterData(chapter, sourceManga));
				return [...volumeChapters, ...singleChapters];
			});
		}
		parseChapterDetails(json, chapterId) {
			const mangaEntry = json.find((entry) => entry.kind === "manga");
			const chapterEntry = json.find((entry) => entry.kind === "chapter");
			if (!mangaEntry || !chapterEntry) throw new Error("Manga o capitolo non trovati nel JSON");
			...
			return {
				id: chapterId,
				mangaId: mangaID,
				pages: info?.pages.map((page) => `${cdnUrl}/chapters/${slug}-${info.mangaId}/${info.chapterURL}/${page}`) ?? []
			};
		}
		/**
		* Page Parsing
		...
		*/
		parsePage(json) {
			return json.filter((entry) => entry.kind === "search").flatMap((entry) => entry.data.mangas.map((manga) => ({
		...
		*/
		async parseSearchResults(excluded, source, metadata, json) {
			const page = metadata?.page ?? 1;
			const totalPages = json.find((item) => item.kind === "searchInfo")?.data.totalPages ?? 1;
			return {
				items: this.parsePage(json).filter((item) => !types.excludedTypes(item.type, excluded.tipi) && !tags.excludedTags(item.tags, excluded.generi)).map((item) => ({
					imageUrl: item.image,
					title: item.title,
					subtitle: item.authors,
					mangaId: item.id,
					contentRating: source.defaultContentRating === ContentRating.ADULT ? ContentRating.ADULT : tags.getRating(item.tags)
				})),
				metadata: page + 1 > totalPages ? void 0 : { page: page + 1 }
			};
		}
		async parseTypeSection(source, metadata) {
			await filter.populateFilter(source);
			const mangaType = [];
			filter.getMangaTypeFilter().filter((option) => !types.blacklistedType(option.value)).forEach((filterItem) => {
				const getExcludedTypeObject = {
					...Object.fromEntries(filter.getMangaTypeFilter().filter((option) => types.blacklistedType(option.value)).map((item) => [item.id, "excluded"])),
					[filterItem.id]: "included"
				};
				mangaType.push({
					type: "genresCarouselItem",
					searchQuery: {
						title: "",
						metadata: { type: getExcludedTypeObject }
					},
		...
		async parseGenreSection(source, metadata) {
			await filter.populateFilter(source);
			const allGenres = [];
			filter.getGenreFilter().filter((option) => !tags.blacklistedTags([option.id])).forEach((filterItem) => {
				const getExcludedValueObject = {
					...Object.fromEntries(filter.getGenreFilter().filter((option) => tags.blacklistedTags([option.id])).map((item) => [item.id, "excluded"])),
					[filterItem.id]: "included"
				};
				allGenres.push({
					type: "genresCarouselItem",
					searchQuery: {
						title: "",
						metadata: { genres: getExcludedValueObject }
					},
					name: filterItem.value,
					metadata,
					contentRating: source.defaultContentRating === ContentRating.ADULT ? ContentRating.ADULT : tags.getRating([filterItem.value])
				});
			});
			return {
				items: allGenres,
		...
		*/
		parseTrendingChapters(metadata, source, chapters) {
			return {
				items: chapters.map((chapter) => ({
					metadata,
					type: "featuredCarouselItem",
					contentRating: source.defaultContentRating === ContentRating.ADULT ? ContentRating.ADULT : source.defaultContentRating,
		...
		*/
		parseMonthTrending(metadata, source, mangas) {
			return {
				items: mangas.map((manga) => ({
					metadata,
					type: "prominentCarouselItem",
					contentRating: source.defaultContentRating === ContentRating.ADULT ? ContentRating.ADULT : tags.getRating(manga.genres?.map((g) => g.slug) ?? []),
		...
		*/
		async parseMostReadSection(metadata, source) {
			let page = metadata?.page ?? 1;
			const $ = await source.requestManager.parsePopularSectionRequests(page, source);
			page++;
			const windowEntry = jsonParser.getWindowEntry($);
			return {
				items: await this.parseSection(page, source, windowEntry),
				metadata: { page }
			};
		}
		async parseLastAddedSection(metadata, source, favTags) {
			let page = metadata?.page ?? 1;
			const html = await source.requestManager.parseLastMangaAddedTagsSectionRequests(page, source, favTags);
			page++;
		...
		async parseSection(page, source, json) {
			const latest = [];
			const parse = this.parsePage(json);
			for (const item of parse) if (!tags.blacklistedTags(item.tags) && !types.blacklistedType(item.type)) latest.push({
				metadata: { page },
				subtitle: item.authors,
		...
		*/
		async parseChapterUpdateSection(metadata, source) {
			const page = metadata?.page ?? 1;
			let html = "";
			const updates = [];
			if (page == 1) html = Application.arrayBufferToUTF8String(await source.requestManager.fetchPage(source.base_url));
			else {
				const data = (await Application.scheduleRequest({
...
//#region node_modules/dom-serializer/node_modules/entities/lib/esm/escape.js
	...
	const getCodePoint$1 = String.prototype.codePointAt != null ? (str, index) => str.codePointAt(index) : (c, index) => (c.charCodeAt(index) & 64512) === 55296 ? (c.charCodeAt(index) - 55296) * 1024 + c.charCodeAt(index + 1) - 56320 + 65536 : c.charCodeAt(index);
	/**
	...
	*/
	const escapeUTF8 = getEscaper$1(/[&<>'"]/g, xmlCodeMap);
	/**
...
/**
	...
	*/
	function getElementsByClassName(className, nodes, recurse = true, limit = Infinity) {
		return filter$3(getAttribCheck("class", className), nodes, recurse, limit);
	}
	/**
...
//#region node_modules/cheerio/dist/browser/options.js
	const defaultOpts$1 = { _useHtmlParser2: false };
	/**
...
//#region node_modules/htmlparser2/node_modules/entities/dist/esm/decode-codepoint.js
	...
	*/
	const fromCodePoint$1 = (_a$2 = String.fromCodePoint) !== null && _a$2 !== void 0 ? _a$2 : ((codePoint) => {
		let output = "";
		if (codePoint > 65535) {
			codePoint -= 65536;
...
//#region node_modules/cheerio/dist/browser/api/attributes.js
	...
	*/
	var _a$1;
	const hasOwn = (_a$1 = Object.hasOwn) !== null && _a$1 !== void 0 ? _a$1 : ((object, prop) => Object.prototype.hasOwnProperty.call(object, prop));
	const rspace = /\s+/;
...
//#region node_modules/cheerio-select/lib/esm/index.js
	...
	*/
	...
	function find$1(root, query, limit = Infinity) {
		const elems = prepareContext(root, esm_exports, query.shouldTestNextSiblings);
...
/**
	...
	*/
	...
	function filterArray(nodes, match, xmlMode, root) {
		return typeof match === "string" ? filter$2(match, nodes, {
			xmlMode,
			root
...
//#endregion
//#region node_modules/parse5/node_modules/entities/dist/esm/decode-codepoint.js
	var _a;
...
//#endregion
//#region node_modules/parse5/node_modules/entities/dist/esm/escape.js
	const getCodePoint = String.prototype.codePointAt == null ? (c, index) => (c.charCodeAt(index) & 64512) === 55296 ? (c.charCodeAt(index) - 55296) * 1024 + c.charCodeAt(index + 1) - 56320 + 65536 : c.charCodeAt(index) : (input, index) => input.codePointAt(index);
	/**
	* Creates a function that escapes all characters matched by the given regular
	* expression using the given map of characters to escape to their entities.
	*
...
//#endregion
//#region node_modules/cheerio/dist/browser/load-parse.js
...
//#endregion
//#region src/generic/utils.ts
	var Tags = class {
		constructor() {
			_defineProperty(this, "excludedTags", (tags, exc) => {
				return tags.some((tag) => {
					return exc.includes(tag);
				});
			});
			_defineProperty(this, "blacklistedTags", (tags) => {
		...
		getRating(tags) {
			for (const tag of tags) {
				const matchedRating = this.tagRatingMap[tag.toUpperCase()] ?? void 0;
	...
	var Type = class {
		constructor() {
			_defineProperty(this, "excludedTypes", (type, excluded) => {
				return excluded.includes(type.toLowerCase());
			});
			_defineProperty(this, "blacklistedType", (type) => {
				return (Application.getState("hide_type") ?? []).includes(type.toLowerCase());
			});
		}
	};
	var FilterPreferences = class {
		constructor() {
			_defineProperty(this, "YearFilter", []);
			_defineProperty(this, "GenreFilter", []);
			_defineProperty(this, "MangaTypeFilter", []);
			_defineProperty(this, "OrderFilter", []);
			_defineProperty(this, "StatusFilter", []);
		}
		/**
		* Set Manga Type Filter
		*/
		setMangaTypeFilter(newValue) {
		...
		*/
		async populateFilter(source, force = false) {
			if (Number(Application.getState("last-filter-fetch") ?? 0) + 604800 > (/* @__PURE__ */ new Date()).valueOf() / 1e3 && !force) {
				const [genres, type, status, sort, year] = [
					"genres",
					"type",
					"status",
					"sort",
				...
				if (genres === void 0 || type === void 0 || status === void 0 || sort === void 0 || year === void 0) {
					await this.populateFilter(source, true);
					return;
				}
				this.setGenreFilter(JSON.parse(genres));
				this.setMangaTypeFilter(JSON.parse(type));
				this.setStatusFilter(JSON.parse(status));
				this.setOrderFilter(JSON.parse(sort));
				this.setYearFilter(JSON.parse(year));
			} else {
		...
		mapStringToOptionItem(tags) {
			if (!tags) return [];
			return tags.map((v) => String(v)).map((tag) => ({
				id: tag,
				value: tag
			}));
		}
		...
		*/
		...
		extractOptionJSON(json) {
			...
			json.forEach((item) => {
				if (item.kind == "global") filters.genres = this.mapGenresToOptionItem(item.data.globalData.genres);
				if (item.kind == "search") {
	...
	var JsonParser = class {
		...
		convertEntries(w) {
			return w.map((entry) => {
				...
				if (typeof data === "object" && data !== null) {
					if (this.isMangaData(data)) return {
						kind: "manga",
						key,
						index,
						data,
					...
					if (this.isGlobalData(data)) return {
						kind: "global",
						key,
						index,
						data,
					...
					if (this.isTrendingData(data)) return {
						kind: "trending",
						key,
						index,
						data,
					...
					if (this.isMangaChapterData(data)) return {
						kind: "chapter",
						key,
						index,
						data,
					...
					if (this.isSearchData(data)) return {
						kind: "search",
						key,
						index,
						data,
					...
					if (this.isSearchInfoData(data)) return {
						kind: "searchInfo",
						key,
						index,
						data,
		...
		getWindowEntry(html) {
			const match = html.match(/<script[^>]*>\s*[^<]*?\$MC\s*=\s*\(window\.\$MC\|\|\[\]\)\.concat\(([\s\S]*?)\)\s*<\/script>/i);
			if (!match?.[1]) throw new Error("No JSON Found");
			const jsonText = match[1].trim();
			const json = JSON.parse(jsonText);
			return this.convertEntries(json.o.w);
		}
		mapGenresToTags(genres) {
			return genres.map((genre) => ({
				id: genre.slug,
				title: genre.name
			}));
		}
		findChapterData(page, chapterId) {
			if (page.volumes.length > 0) for (const volume of page.volumes) {
...
//#endregion
//#region src/generic/main.ts
	const filter = new FilterPreferences();
	const tags = new Tags();
	const types = new Type();
	const jsonParser = new JsonParser();
	var MangaWorldGeneric = class {
		constructor(params) {
			_defineProperty(this, "name", void 0);
			_defineProperty(this, "base_url", "");
			_defineProperty(this, "defaultContentRating", ContentRating.EVERYONE);
			_defineProperty(this, "parser", void 0);
			_defineProperty(this, "requestManager", void 0);
			_defineProperty(this, "mainRateLimiter", void 0);
			_defineProperty(this, "mainInterceptor", void 0);
		...
		async getSearchResults(query, metadata, sorting) {
			...
			const page = metadata?.page ?? 1;
			const { url, excluded } = this.requestManager.constructSearchRequestURL(page, query, sorting, this);
		...
		async getMangaDetails(mangaId) {
			...
			const windowEntry = jsonParser.getWindowEntry(html);
			return this.parser.parseMangaDetails(windowEntry, mangaId, `${this.base_url}/manga/${mangaId}`, this);
		}
		...
		async getChapterDetails(chapter) {
			const data = this.requestManager.fetchPage(`${this.base_url}/manga/${chapter.sourceManga.mangaId}`);
			const html = Application.arrayBufferToUTF8String(await data);
			const windowEntry = jsonParser.getWindowEntry(html);
		...
		async getDiscoverSections() {
			...
			if (Application.getState("popular_section_enabled") ?? true) discover_section.push({
				id: "popular_section",
				title: "Capitoli In Tendenza",
				type: DiscoverSectionType.featured
			});
			if (Application.getState("mese_section_enabled") ?? true) discover_section.push({
				id: "mese_section",
				title: "Tendenze del Mese",
				subtitle: "Più letti del mese",
				type: DiscoverSectionType.prominentCarousel
			});
			if (Application.getState("most_read_section_enabled") ?? true) discover_section.push({
				id: "most_read_section",
				title: "Più Letti",
				subtitle: "I più popolari di sempre",
				type: DiscoverSectionType.simpleCarousel
			});
			if (Application.getState("new_section_enabled") ?? true) discover_section.push({
				id: "new_manga_section",
				title: "Nuove Aggiunte",
				subtitle: "Le nuove Aggiunte",
				type: DiscoverSectionType.simpleCarousel
			});
			if (Application.getState("update_section_enabled") ?? true) discover_section.push({
				id: "updated_section",
				title: "Aggiornati di Recente",
				subtitle: "Ultimi Capitoli Aggiunti",
				type: DiscoverSectionType.chapterUpdates
			});
			if ((Application.getState("fav_tags_new")?.length ?? 0) > 0 && (Application.getState("fav_section_enabled") ?? true)) discover_section.push({
			...
			if (Application.getState("type_section_enabled") ?? true) discover_section.push({
				id: "type_section",
				title: "Tipologia",
				subtitle: "Più letti di una tipologia",
				type: DiscoverSectionType.genres
			});
			if (Application.getState("genre_section_enabled") ?? true) discover_section.push({
				id: "genre_section",
				title: "Genere",
		...
		async getSection(id, json, metadata) {
			let section = {
				items: [],
				metadata
			};
			...
			if (id === "popular_section" || id === "mese_section") for (const item of json) {
...
//#endregion
//#region src/MangaWorldAdult/pbconfig.ts
...
//#endregion
//#region src/MangaWorldAdult/main.ts
	const DOMAIN = "https://www.mangaworldadult.net";
	var MangaWorldAdultExtension = class extends MangaWorldGeneric {
		constructor() {
			super({
				domain: DOMAIN,
				name: pbConfig.name,
	...
	const MangaWorldAdult = new MangaWorldAdultExtension();
...
//#endregion
//#region src/tests/MangaWorldAdult.ts
	async function runTests(logger) {
		const suite = new TestSuite("MangaWorldAdult tests", logger);
		registerDefaultTests(suite, MangaWorldAdult, pbConfig);
		await suite.run();
	}
...
//# sourceMappingURL=MangaWorldAdult.js.map